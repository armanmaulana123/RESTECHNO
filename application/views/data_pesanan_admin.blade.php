						...
						<tbody>
							...
							<tr align="center">
								...
								<td>
									...
									<a class="btn btn-sm btn-warning" href="{{ base_url('akun/pesanan/') . $p['kode_transaksi'] }}" title="Menu Yang Dipesan"><i class="fas fa-shopping-cart text-light"></i></a>
									@if ($p['status'] != 'belum dibayar')

									@if ($p['nama_metode'] == 'Rekening')

									<a class="btn btn-sm btn-success"
										href="{{ base_url('akun/bukti_pembayaran_admin/') . $p['kode_transaksi'] }}" title="Bukti Pembayaran"><i
											class="fas fa-file"></i></a>

									@endif
									
									@if ($p['status'] == 'diproses')
									@if ($p['nama_metode'] == 'Tunai')
									<form action="{{ base_url('akun/konfirmasi_pembayaran') }}" method="POST">
									<input type="hidden" name="kode_transaksi" value="{{ $p['kode_transaksi'] }}">
