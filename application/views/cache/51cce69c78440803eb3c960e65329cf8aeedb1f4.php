		...
		<div class="modal-content">
			...
			<div class="modal-footer">
				...
				<a class="btn btn-primary" href="javascript:void(0)"
					onclick="detail_keranjang('<?php echo e($p['kode_transaksi']); ?>')">Data Keranjang</a>
			</div>
		</div>
	</div>
</div>
...
<!-- Modal Data Pesanan -->
<div class="modal fade" id="modal_form_keranjang" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
	aria-hidden="true">
	<div class="modal-dialog modal-dialog-centered modal-lg" role="document">
		<div class="modal-content">
			<div class="modal-header" style="background : linear-gradient(120deg, #3498db, #8e44ad)">
				<h5 class="modal-title text-light" id="exampleModalCenterTitle">Detail Keranjang</h5>
				<button type="button" class="close text-light" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
...
<script>
	...
	function detail_keranjang(id) {
		$('.form-group').removeClass('has-error'); // clear error class
		$('.help-block').empty(); // clear error string
		$('#modal_form_keranjang').modal('show'); // show bootstrap modal when complete loaded

		//Ajax Load data from ajax
		$.ajax({
			url: "<?php echo site_url('akun/tampil_detail_keranjang/')?>/" + id,
			type: "GET",
			dataType: "JSON",
			success: function (data) {

				$('[name="nama_menu"]').val(data.nama_menu);
				$('[name="image_menu"]').val(base_url('assets/img/daftar_menu/').data.image_menu);
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layout.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\restechno\application\views/data_pesanan_chef.blade.php ENDPATH**/ ?>